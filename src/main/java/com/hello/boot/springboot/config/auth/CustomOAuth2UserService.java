...
import com.hello.boot.springboot.config.auth.dto.OAuthAttributes;
import com.hello.boot.springboot.config.auth.dto.SessionUser;
import com.hello.boot.springboot.domain.user.User;
import com.hello.boot.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        /*
        * OAuth2 로그인 진행 시 키가 되는 필드값을 이야기 한다 PK와 같은 의미
        * 구글의 경우 기본적으로 코드를 지원하지만 네이버, 카카오는 지원 X 구글의 기본 코드는 'sub'
